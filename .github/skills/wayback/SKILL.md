...
Search and browse archived plans stored in `/memories/repo/plans/archive/`. Each archive contains the full plan (MVTs, dispatch order, implementation commit) and its critique findings.
...
Read the relevant archive(s) using `memory view`. Answer the user's question by referencing specific MVTs, critique findings, or implementation details from the score.

Common queries:
- **"What went wrong with X?"** — Read the critique's "What Didn't" section and find MVTs touching the relevant area.
- **"How did we handle X last time?"** — Find MVTs by owned files or intent keywords.
...
If the user is asking in the context of planning new work, highlight relevant lessons: MVT shapes that worked or failed, scoping patterns, owned-file mistakes, or process gaps that were corrected.
