const CACHE_NAME = 'jelly-wobble-v1'
const BASE_PATH = self.location.pathname.replace(/\/jelly-wobble\/sw\.js$/, '') || ''
function withBase(url) { return url.startsWith('/') ? BASE_PATH + url : url }
self.addEventListener('install', () => { self.skipWaiting() })
self.addEventListener('activate', (event) => { event.waitUntil(caches.keys().then(keys => Promise.all(keys.filter(k => k.startsWith('jelly-wobble-') && k !== CACHE_NAME).map(k => caches.delete(k))))) ; self.clients.claim() })
