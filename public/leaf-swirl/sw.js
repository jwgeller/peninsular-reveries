const CACHE_NAME = 'leaf-swirl-v1'
const BASE_PATH = self.location.pathname.replace(/\/leaf-swirl\/sw\.js$/, '') || ''
function withBase(url) { return url.startsWith('/') ? BASE_PATH + url : url }
self.addEventListener('install', () => { self.skipWaiting() })
self.addEventListener('activate', (event) => { event.waitUntil(caches.keys().then(keys => Promise.all(keys.filter(k => k.startsWith('leaf-swirl-') && k !== CACHE_NAME).map(k => caches.delete(k))))) ; self.clients.claim() })
